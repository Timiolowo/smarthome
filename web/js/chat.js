...
async function submitPrompt() {
  ...
  try {
    ...
    asstWrap.innerHTML = `
      <div class="chat-bubble">${replyText}</div>
    ...
    if (replyLine) {
      replyLine.textContent = `"${cleanSpokenText(replyText)}"`;
    }
    ...
    errWrap.innerHTML = `
      <div class="chat-bubble" style="color:hsl(var(--destructive));">Error communicating with assistant: ${e.message}</div>
    `;
