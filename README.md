...
- **🎙️ Natural Voice Stack**: Offline speech-to-text via Faster-Whisper, real-time microphone voice activity detection (VAD), and flexible text-to-speech (macOS `say`, Piper TTS, pyttsx3, and browser Web Speech).
- **📶 Passive Presence Detection**: Automatic arrival and departure awareness via Wi-Fi network polling (ICMP ping & ARP fallback) without draining phone battery or requiring a mobile app.
...
The easiest and fastest way to get started. The installer automatically detects your OS, creates a Python virtual environment, installs dependencies, initializes configs, and offers to download offline AI models:
...
# Copy example environment variables (for cloud LLM API keys)
cp .env.example .env
```
...
| Setting | Description |
| :--- | :--- |
| `user_name` | Your name (used for personalized memory & greetings). |
...
- **Conversations & Questions**: "What's the weather like today?", "Who is Arthur Pendragon?", "Summarize what we discussed yesterday."
